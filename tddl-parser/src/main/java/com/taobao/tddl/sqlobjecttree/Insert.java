	...
	public Map<String, Comparative> getSubColumnsMap() {
		...
		for(int i=0;i<valueObj.size();i++){
			...
			if(val instanceof Value){
				temp=((Value)val).eval();
				map.put(col.getColumn().toLowerCase(),new Comparative(Comparative.Equivalent,temp));
			}else if(val instanceof Comparable){
				map.put(col.getColumn().toLowerCase(), new Comparative(Comparative.Equivalent,(Comparable)val));
			}
		}
		return map;
	}
...
//		columns.regTableModifiable(oraTabName, list);
//		for(Object val:valueObj){
//			ModifiedTableUtils.regModifiedTab(val, oraTabName, list);
//		}
//	}
	@Override
	...
	protected StringBuilder appendInsertBody(String oraTabName,
