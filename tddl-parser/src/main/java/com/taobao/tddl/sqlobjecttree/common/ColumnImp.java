...
public class ColumnImp implements Column,SubQueryValue{
	...
	public StringBuilder regTableModifiable(String oraTabName, List<Object> list,
		...
		if (table != null) {
			if(table.equalsIgnoreCase(oraTabName)){
				...
				wrapper.setOriTable(oraTabName);
				list.add(wrapper);
	...
	public Comparable<?> getVal(List<Object> args) {
		...
		fragement = aliasMap.get(table);

		if(fragement!=null){
			if(fragement instanceof TableNameFunctionImp){
				Function func = ((TableNameFunctionImp)fragement).getFunction();
				return func.getVal(args);
			}else if(fragement instanceof TableNameSubQueryImp){
				Select subSelect = ((TableNameSubQueryImp)fragement).getSubSelect();
				TableNameFunctionImp tbNameFunc=subSelect.getTableNameFunction();
				if(tbNameFunc==null){
					throw new IllegalArgumentException("列名字段:"+table+"."+column+"不能映射到一个可以赋值的" +
							"虚拟列名中");
				}else{
