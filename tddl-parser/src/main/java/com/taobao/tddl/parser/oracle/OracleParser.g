...
likeCondition
	:'NOT''LIKE'  like_value->^(NOT_LIKE like_value)
	|'LIKE' like_value->^(LIKE like_value)
	;
	
like_value
	: single_concat_value
	| multi_concat_values->^(MULTI_STR multi_concat_values)
	;
	
concat_values_func
	:lu_value COMMA! lu_value
	|single_concat_value COMMA! lu_value
	|lu_value COMMA! single_concat_value
	;	

single_concat_value
	:{functionMap.containsKey(input.LT(1).getText().toUpperCase())}? ID (LPAREN concat_values_func? RPAREN)->^(ID concat_values_func?)
	;
