...
List<Object> list=new ArrayList<Object>();
	...
	|^(MULTI_STR (expr[$where]{list.add($expr.valueObj);})* ){
...
table_spec[DMLCommon common]
	...
	{
		common.addTableNameAndSchemaName($table_name.text,$schema_name.text,$alias.aliText, true);
	}
	|subquery[$common.getIndexHolder()] alias?
	{
		common.addTableSubQuery($subquery.subselect,$alias.aliText, true);
	...
	|^(ID (expr[$common.getIndexHolder()]{list.add($expr.valueObj);})  alias?)
	{
		Function func=functionMap.get(($ID.text).toUpperCase());
		func.setValue(list);
		common.addTableFunction(func,$alias.aliText, true);
	}
	;
