...
public class MockDataSource implements DataSource, Cloneable{
	...
	private void throwFatalSQLException() throws SQLException {
		if (DBType.MYSQL.equals(dbType)) {
			throw new SQLException("dsClosed", "08001");//来自MySQLExceptionSorter
		} else if (DBType.ORACLE.equals(dbType)) {
			throw new SQLException("dsClosed", "28");//来自OracleExceptionSorter //28 session has been killed
		} else {
	...
	public static void showTrace(String msg){
		...
		System.out.println("---------------end of shwo Trace-------------------");
	}
	...
	public static boolean hasTrace(String dbIndex, String sqlHead) {
		...
		if (trace != null) {
			for (ExecuteInfo info : trace) {
				if (info.sql != null && dbIndex.equals(info.ds.dbIndex)
	...
	public Connection getConnection() throws SQLException {
		...
		return new MockConnection("getConnection",this);
	}
