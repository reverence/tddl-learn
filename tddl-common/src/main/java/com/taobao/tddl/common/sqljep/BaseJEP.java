...
import com.taobao.tddl.common.sqljep.function.PostfixCommandI;
 ...
 * source data. <p/> There are two types of variables in an expression. Columns
 * of a abstarct table data and external variables. Example of external
 * variables are <CODE>TIMESTAMP,DATE,TIME,SYSDATE</CODE> variables. <p/> User
 * should override methods {@link #findColumn(String name)} and
 ...
 * variables. <p/> To get a variable value at first
 * {@link #getVariable(String name)} and after that
 * {@link #findColumn(String name)} method is used. <p/> In expressions
 * 'part_1.part2' names of variables are supported. Parser is case sensitive.
 * <p/> Visit <a
 * href="http://sourceforge.net/projects/sqljep">http://sourceforge
 * .net/projects/sqljep</a> for the newest version of SQLJEP, and complete
 * documentation.
 *
...
public abstract class BaseJEP implements ParserVisitor {
	 ...
	 * expression. <br/> To compile it use {@link #parseExpression} method.<br/>
	 ...
	 * expression, and subsequently evaluate it manually. <p/> Sometimes it is
	 * necessary to change value of a SQLJEP expression by some external
	 * reasons. Then following code can be applied <blockquote>
	 *
	 * <pre>
	 ...
	 * } else {
	 * 	throw new org.medfoster.sqljep.ParseException(
	 ...
	 * @param column
	 *            Number in the abstract table data
	 * @throws com.taobao.tddl.common.sqljep.ParseException
	 */
	@SuppressWarnings("unchecked")
	public abstract Comparable getColumnObject(int column)
	 ...
	 *
	 * @throws com.taobao.tddl.common.sqljep.ParseException
	 *             If there are errors in the expression then it fires the
	 *             exception
	 */
	final public void parseExpression(Map<String, Integer> columnMapping,
									  Map<String, Variable> variableMapping,
									  final Map<String, PostfixCommand> funMap) throws ParseException {
		...
		try {
			...
			throw new com.taobao.tddl.common.sqljep.ParseException(toString(),
					e);
		}

		if (hasError()) {
			throw new com.taobao.tddl.common.sqljep.ParseException(
					getErrorInfo());
		}
	 ...
	 * {@link com.taobao.tddl.common.sqljep.JepRuntime#stack} for evaluation.
	 * <p>
	...
	public Comparable<?> getValue(Comparable<?>[] row) throws ParseException {
		...
		if (!hasError()) {
			...
			if (runtime.stack.size() != 1) {
				throw new ParseException("Wrong stack state. Stack size: "
						+ runtime.stack.size());
			}
	...
	public Comparable<?> getValue(Comparable<?>[] row,Config config)throws ParseException{
		NestThreadLocalMap.put(StaticString.CONFIG, config);
		Comparable<?> m=null;
		try{
			m= getValue(row);
		}finally{
			NestThreadLocalMap.put(StaticString.CONFIG, null);
		}
		return m;
	}
	 ...
	 */
	final public Object visit(SimpleNode node, Object data)
			throws ParseException {
		return null;
	}
	 ...
	 * ============================
	 */
	final public Object visit(ASTFunNode node, Object data)
			throws ParseException {
		JepRuntime runtime = getThreadJepRuntime(this);
		...
		if (debug) {
			System.out.println("Stack size after childrenAccept: "
					+ runtime.stack.size());
		}

		Comparable<?>[] parameters = pfmc.evaluate(node, runtime);

		computeComparative(runtime, pfmc, parameters);
	...
	private void computeComparative(JepRuntime runtime, PostfixCommandI pfmc,
									Comparable<?>[] parameters) throws ParseException {
		if (pfmc.isAutoBox()) {
			//查看内部是否有List表达式
			...
			if (index >= 0) {
				computeComparativeBaseList(runtime, pfmc, parameters, list,
						index);
			} else {
				...
				for (int i = 0; i < parameters.length; i++) {
					if (parameters[i] instanceof Comparative) {
						...
						parameters[i] = ((Comparative) parameters[i])
								.getValue();
					}
				}
	...
	private void computeComparativeBaseList(JepRuntime runtime,
											PostfixCommandI pfmc, Comparable<?>[] parameters,
											ComparativeBaseList list, int index) throws ParseException {
		for (int i = 0; i < parameters.length; i++) {
			if (i != index) {
				if (parameters[i] instanceof Comparative) {
					parameters[i] = ((Comparative) parameters[i])
		...
		getEvaluatedComparable(pfmc, parameters, list, index);
		runtime.stack.push(list);
	}

	private void getEvaluatedComparable(PostfixCommandI pfmc,
										Comparable<?>[] parameters, ComparativeBaseList list, int index)
			throws ParseException {
		...
		for (Comparative comp : list.getList()) {
			...
			if(parameters[index]!=null){
				value = pfmc.getResult(parameters);
			}else{
				value=comp;
			}
			if (value instanceof ComparativeBaseList) {
				ComparativeBaseList sub = (ComparativeBaseList) value;
				getEvaluatedComparable(pfmc, parameters, sub, index);
	 ...
	 */
	@SuppressWarnings("unchecked")
	final public Object visit(ASTVarNode node, Object data)
			throws ParseException {
		JepRuntime runtime = getThreadJepRuntime(this);
		if (node.index >= 0) {
	 ...
	 * stack.
	 */
	final public Object visit(ASTConstant node, Object data)
			throws ParseException {
		JepRuntime runtime = getThreadJepRuntime(this);
	...
	public static JepRuntime getThreadJepRuntime(BaseJEP baseJep) {
		JepRuntime runtime = (JepRuntime) NestThreadLocalMap
				.get(StaticString.JEP_RUNTIME);
		if (runtime == null) {
			runtime = new JepRuntime(baseJep);
			NestThreadLocalMap.put(StaticString.JEP_RUNTIME, runtime);
		} else {
			runtime.ev = baseJep;
		}
		return runtime;
