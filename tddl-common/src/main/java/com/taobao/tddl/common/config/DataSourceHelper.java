...
public class DataSourceHelper {
	...
	public static LocalTxDataSource findDataSource(DataSourceConfig config, Map<String, DataSourceConfig> dataSourceConfigs) {
		...
		for (Entry<String,DataSourceConfig> entry : dataSourceConfigs.entrySet()) {
			...
			LocalTxDataSourceDO oldConfig = dataSourecConfig.getLocalTxDataSourceDO();
			//判断必要参数是否相当，如果这些必要的参数都相等则认为是同一个数据源
			LocalTxDataSourceDO	nowConfigDO=config.getLocalTxDataSourceDO();
			if (StringUtil.equals(oldConfig.getConnectionURL(), nowConfigDO.getConnectionURL())
				...
				return dataSourecConfig.getLocalTxDataSource();
			}
		}
		return null;
	}
	...
	public static DataSource buildDataSource(DataSourceConfig config) {
		if("jndi".equalsIgnoreCase(config.getType())) {
			...
			try {
				...
				return (DataSource) context.lookup(config.getLocalTxDataSourceDO().getJndiName());
		...
		try {
			LocalTxDataSource localDataSource=TaobaoDataSourceFactory.createLocalTxDataSource(config.getLocalTxDataSourceDO());
			config.setLocalTxDataSource(localDataSource);
	...
	public static Map<String, DataSource> buildDataSource(Map<String, DataSourceConfig> configs, Map<String, DataSourceConfig> oldConfigs) {
		...
		if (null != oldConfigs) {
			...
			for (DataSourceConfig oldDataSourceConfig : oldConfigs.values()) {
				LocalTxDataSource oldLocalTxDataSource = oldDataSourceConfig
						.getLocalTxDataSource();
				if (null != oldLocalTxDataSource) {
					oldDataSourceList.add(oldLocalTxDataSource);
				}
			}
