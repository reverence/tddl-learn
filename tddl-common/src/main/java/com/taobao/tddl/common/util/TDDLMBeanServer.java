...
public final class TDDLMBeanServer {
	private static final Logger log = Logger.getLogger(TDDLMBeanServer.class);
	...
	private TDDLMBeanServer() {
		...
		try {
			...
			if (useJmx) {
				...
				final JMXConnectorServer connectorServer = JMXConnectorServerFactory.newJMXConnectorServer(url, null,
						mbs);
