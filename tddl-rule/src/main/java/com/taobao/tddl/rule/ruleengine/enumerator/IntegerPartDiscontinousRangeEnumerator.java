...
public class IntegerPartDiscontinousRangeEnumerator extends
	...
	protected Comparative changeGreater2GreaterOrEq(Comparative from) {
		if (from.getComparison() == Comparative.GreaterThan) {
			...
			return new Comparative(Comparative.GreaterThanOrEqual,
					fromComparable + LIMIT_UNIT_OF_INT);
		} else {
			return from;
		}
	}
	...
	protected Comparative changeLess2LessOrEq(Comparative to) {
		if (to.getComparison() == Comparative.LessThan) {
			...
			return new Comparative(Comparative.LessThanOrEqual, toComparable
					- LIMIT_UNIT_OF_INT);
		} else {
	 ...
	 * 
	 * @see
	 * com.taobao.tddl.rule.ruleengine.enumerator.PartDiscontinousRangeEnumerator
	 * #getAllPassableFields(com.taobao.tddl.common.sqljep.function.Comparative,
