...
import com.taobao.tddl.common.config.beans.Preffix;
...
/**
 * 根据用户配置的一段groove脚本返回一个List<string>
 * 直接拼成一个map返回
 * @author zuomou
 *
 */
public class GroovyTableDatabaseMapProvider extends SimpleTableMapProvider {
    ...
    private static final Log           log                = LogFactory
            .getLog(GroovyTableDatabaseMapProvider.class);
    ...
    protected List<String> getTableNames(String tableFactory, String expression)
        ...
        log.warn("tbsuffix shell use time:" + (time2 - time1) + ",expression=" + expression);
        ...
        if (value instanceof List) {
            ...
            log.warn("wocao, dbId=" + super.getParentID() + ",tableNameList=" + finalList);

           /* //如果指定每个分表库分表的个数为-1，说明每个库都是初始化所有的表
             if (super.doesNotSetTablesNumberForEachDatabases()) {
                 String temp = this.getPadding();
                 for (String tname : tableNameList) {
                     finalList.add(new StringBuilder(tableFactory).append(temp).append(tname)
                         .toString());
    ...
    public Map<String, SharedElement> getSharedElemenetMapBySharedElementList(
        ...
        if (sharedElementList != null) {
            for (SharedElement sharedElement : sharedElementList) {
                ...
                //String key = t.getTableName().substring(getTableFactor().length());
     ...
     *
     * @param dbNumber value to be assigned to property dbNumber
     */
     ...
     *
     * @param dbAndNumberRelation value to be assigned to property dbAndNumberRelation
     */
