...
public class SimpleLogicTable extends LogicTable{
	private static final Log log = LogFactory.getLog(SimpleLogicTable.class);
	...
	protected void valid(int databasesSize, int tableSizeForEachDatabase) {
		...
		if(dividend % divisor != 0){
			throw new IllegalArgumentException("分表个数必须是分库个数的倍数," +
					"分库是:"+databasesSize+"分表是:"+tableSizeForEachDatabase);
		}
	}

	static class TableAGRuleHandler extends DatabaseAndTableAGRuleHandler implements AutoGenerationRuleHandler{
		...
		public String getTableRule(String tableShardingKey, int databaseSize,
			...
			if(tableSizeForEachDatabase != 0){
				sb.append(" % ")
						.append(tableSizeForEachDatabase);
			}
			return sb.toString();
		}
		public String getDatabaseRule(String databaseShardingKey,
									  int tablesSize, int tableSizeForEachDatabase) {
			return null;
		}
	}
	static class DatabaseAGRuleHandler extends DatabaseAndTableAGRuleHandler implements AutoGenerationRuleHandler{
		@Override
		public String getTableRule(String tableShardingKey, int databaseSize,
								   int tableSizeForEachDatabase) {
			return null;
	...
	static class DatabaseAndTableAGRuleHandler implements AutoGenerationRuleHandler{
		...
		public String getDatabaseRule(String databaseShardingKey,int databaseSize,
			...
			sb.append(")");
			if(tableSizeForEachDatabase != 0){
				sb.append(".intdiv(").append(tableSizeForEachDatabase)
						.append(")");
			}
			return sb.toString();
		}
	...
	static interface AutoGenerationRuleHandler{
		 ...
		 * @return
		 */
		String getTableRule(String tableShardingKey,int tablesSize,
							int tableSizeForEachDatabase);
		/**获取分库规则
		 *
		 * @param tableShardingKey 分表键
	...
	protected void processAutoGenerationRule(){
		...
		String dbRule = agrHandler.getDatabaseRule(shardingKey, databaseSize, tablesNumberForEachDatabases);
		...
		if(dbRule != null&& this.databaseRuleStringList == null){
			...
			log.info("auto generation rule for database: "+ dbRule);
			databaseRuleStringList.add(dbRule);
		}
		String tableRule = agrHandler.getTableRule(shardingKey, databaseSize, tablesNumberForEachDatabases);
		if(tableRule != null){
			this.tableRuleStringList = new ArrayList<Object>(1);
			log.info("auto generation rule for database: "+ tableRule);
	...
	protected int getTablesNumberForEachDatabases() {
		...
		if(tablesNumberForEachDatabases == SimpleTableMapProvider.DEFAULT_TABLES_NUM_FOR_EACH_DB){
			...
			tablesNumberForEachDatabases = simpleTableMapProvider.getTo() - simpleTableMapProvider.getFrom() + 1;
		}
	...
	public void init() {
		...
		if(superClassDatabaseRuleIsNull()){
			RuleChain rc = RuleUtils.getRuleChainByRuleStringList(databaseRuleStringList, GroovyListRuleEngine.class,isDatabase);

			super.listResultRule = rc;
		}

		if(transmitterTableRuleIsNull()){
			RuleChain rc = RuleUtils.getRuleChainByRuleStringList(tableRuleStringList, GroovyListRuleEngine.class,!isDatabase);
	...
	public void setSimpleTableMapProvider(
			SimpleTableMapProvider simpleTableMapProvider) {
		this.simpleTableMapProvider = simpleTableMapProvider;
	}

	@Override
	public String toString() {
		return "SimpleLogicTable [databaseRuleStringList="
				+ databaseRuleStringList + ", databases=" + databases
				+ ", isSimpleTableMapPropertiesChanged="
				+ isSimpleTableMapPropertiesChanged + ", shardingKey="
				+ shardingKey + ", simpleDatabaseMapProvider="
				+ simpleDatabaseMapProvider + ", simpleTableMapProvider="
				+ simpleTableMapProvider + ", tableRuleStringList="
				+ tableRuleStringList + ", useAutoGeneratingRule="
				+ useAutoGeneratingRule + ", defaultListResult="
				+ defaultListResult + ", defaultListResultStragety="
				+ defaultListResultStragety + ", listResultRule="
