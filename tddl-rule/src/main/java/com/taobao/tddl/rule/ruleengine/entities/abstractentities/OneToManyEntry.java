...
public class OneToManyEntry {
		...
		if (tableRuleList != null) {
			...
			if (transmitedtableRuleChain == null) {
				...
				throw new IllegalArgumentException(
						"ruleChain已经有数据了，但仍然尝试使用init来进行初始化");
			}
		}
		if (transmitedtableRuleChain != null) {
	...
	public static RuleChainImp getRuleChain(
			List<ListAbstractResultRule> tableRuleList) {
	...
	public void setTableMapProvider(TableMapProvider tableMapProvider) {
		...
		if (tableMapProvider != null
				&& (!(tableMapProvider instanceof DefaultTableMapProvider))) {
