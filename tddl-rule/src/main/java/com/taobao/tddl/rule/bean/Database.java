...
import com.taobao.tddl.rule.ruleengine.rule.Field;
...
public class Database extends ListSharedElement implements
	 ...
	 *            规则和规则对应的上下文。
	 */
	public void calculateTable(TargetDB targetDB,
							   Field sourceTrace,
		...
		if (calculationContext == null) {
			...
			resultSet = rule.evalWithoutSourceTrace(argsFromSQL,
					null,null);
		} else {
			//2.2）传递了定位到本database时的列名->描点的集合
			ListAbstractResultRule rule = calculationContext.ruleChain.getRuleByIndex(calculationContext.index);
...
//				}
		...
		for (String key : resultSet) {
			...
			if (table == null) {
				throw new IllegalArgumentException(
						"cant find table by target index :" + key
	...
	private Map<String, Set<Object>> getEnumeratedSqlArgsMap(
			CalculationContextInternal calculationContext, ListAbstractResultRule rule) {
		if(rule == null){
			throw new IllegalStateException("should not be here");
		}
		//强转 需要用到里面的方法
		Map<String, Set<Object>>  argsFromSQL = RuleUtils.getSamplingField(calculationContext.sqlArgs,rule.getParameters());
		return argsFromSQL;
	}
