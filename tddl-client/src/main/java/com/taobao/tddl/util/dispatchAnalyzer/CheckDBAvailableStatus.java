...
/**
 * 采用异步线程的方式，轮询db的可用状态，只有在状态发生变更的情况下，才进行状态修改；
 * 状态表更包括以下两种情况：第一，db 可用->不可用； 第二， db 不可用-> 可用；
 * 为防止线程池堵死，采用了DiscardOldestPolicy 策略；
...
public class CheckDBAvailableStatus {
    public static final Log logger             = LogFactory.getLog(CheckDBAvailableStatus.class);
    /**
     * TDDL封装的数据源
     */
    ...
    private synchronized void circulateDBStatus() {
        ...
        //外围循环，遍历每一个全活策略的组
        for (Map.Entry<String, TDataSourceKeyWeightRandom> entry : keyWeightMapConfig.entrySet()) {
            boolean changeFlag = false;
            String group_key = entry.getKey().trim();
            TDataSourceKeyWeightRandom tDataSourceKeyWeightRandom = entry.getValue();
                ...
                if (isAvailable) {
                    ...
                    if (weightValues[i] > 0) {
                        if (++number <= getCloseDBLimitNumber(dbNumberInGroup)) {
                            ...
                            weightValue = 0;
                            if (logger.isWarnEnabled()) {
                                logger.warn("The db status will change from YES to NOT,dbName="
                                        + dbKey + ",the total number of " + group_key + " is "
                                        + dbNumberInGroup + ",and the failed number is "
                                        + number);
                            }
                        } else {
                            if (logger.isWarnEnabled()) {
        ...
        if (isDBStatusChanged && logger.isWarnEnabled()) {
            logger.warn("WO CAO,The db status have changed, it is:" + dbKeyWeightToString());
        }
    ...
    private boolean isAvailableDB(final String dbKey) {
        ...
        try {
            ...
            logger.error("获取数据库状态超时:threadName=" + Thread.currentThread().getName()
                    + ",dbKey=" + dbKey + ",cacleResult=" + result + ",workQueueCapacity="
    ...
    private boolean checkDBAvailalbeStatusUtil(final String dbKey) {
        ...
        RuntimeConfigHolder<TddlRuntime> runtimeConfigHolder = getTargetDataSource()
