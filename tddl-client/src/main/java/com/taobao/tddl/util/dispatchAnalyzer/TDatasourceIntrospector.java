...
import com.taobao.tddl.common.config.beans.ShardRule;
...
import com.taobao.tddl.util.IDAndDateCondition.routeCondImp.TableShardingRuleImpl;
...
public class TDatasourceIntrospector {

    public static final Log                     logger                      = LogFactory
            .getLog(TDatasourceIntrospector.class);
    /**
     * TDDL封装的数据源
     */
    ...
    public void init() {
        ...
        checkDBAvailableStatus.setIsUseFutureMode(this.isUseFutureMode);
    ...
    private String[] getAvailableGroupDBAndTableByWeights(String logicTableName, int groupNum,
        ...
        TDataSourceKeyWeightRandom tDataSourceKeyWeightRandom = getTDataSourceKeyWeightRandom(groupNum);
        String dbKey = tDataSourceKeyWeightRandom.getDBKeyByNumber(Integer.valueOf(orderInGroup));
    ...
    private String getAvailableGroupDBByWeights(int groupNum, boolean isCheckConnection) {
        ...
        orderInGroup = tDataSourceKeyWeightRandom
                .getRandomDBIndexByWeight(new ArrayList<Integer>());
        //获取该db对应的标识dbKey
        String dbKey = tDataSourceKeyWeightRandom.getDBKeyByNumber(orderInGroup);

        if (logger.isDebugEnabled()) {
            logger.debug("dbNumber=" + orderInGroup + ",dbKey=" + dbKey);
        }

        if (orderInGroup < 0
                || orderInGroup >= tDataSourceKeyWeightRandom.getDataSourceNumberInGroup()) {
    ...
    private Map<String, TableRule> getTableRules() {
        String[] appRuleFiles = getTargetDataSource().getAppRuleFiles();
        AppRule appRule = (AppRule) getTargetDataSource().getAppRule(appRuleFiles);
        if (appRule == null) {
    ...
    public boolean isDataBaseAvailable(int dbNumber, int groupNum) {
        TDataSourceKeyWeightRandom tDataSourceKeyWeightRandom = getTDataSourceKeyWeightRandom(groupNum);
        //获取该db对应的标识dbKey
        int dbWeight = tDataSourceKeyWeightRandom.getDBWeightByNumber(dbNumber);
    ...
    public List<Integer> getNotAvailableDBIndexes(int groupNum) {
        TDataSourceKeyWeightRandom tDataSourceKeyWeightRandom = getTDataSourceKeyWeightRandom(groupNum);
        return tDataSourceKeyWeightRandom.getNotAvailableDBIndexes();
    }
    ...
    public List<Integer> getAvailableDBIndexes(int groupNum) {
        TDataSourceKeyWeightRandom tDataSourceKeyWeightRandom = getTDataSourceKeyWeightRandom(groupNum);
        return tDataSourceKeyWeightRandom.getAvailableDBIndexes();
    }

    /**
     * 根据组号获取随机器对象
     * @param groupNum     组号<br>
     * @return      TDataSourceKeyWeightRandom对象
     */
    private TDataSourceKeyWeightRandom getTDataSourceKeyWeightRandom(int groupNum) {
        Map<String, TDataSourceKeyWeightRandom> keyWeightMapConfig = getTargetDataSource()
        ...
        String groupNo = DBINDEX_DS_GROUP_KEY_PREFIX + groupNum;
        TDataSourceKeyWeightRandom tDataSourceKeyWeightRandom = keyWeightMapConfig.get(groupNo);
        if (tDataSourceKeyWeightRandom == null) {
            throw new IllegalArgumentException("Please check the configure,the groupNum is "
                    + groupNo);
        }
        return tDataSourceKeyWeightRandom;
    }
    ...
    private DatabaseChoicer getDatabaseChoicer() {
        TDataSource tds = getTargetDataSource();
     ...
     * @return  tddl封装后的数据源
     */
    public TDataSource getTargetDataSource() {
     ...
     */
    public void setTargetDataSource(TDataSource targetDataSource) {
    ...
    /**
     * Getter method for property <tt>checkDBAvailableStatus</tt>.
     *
     * @return property value of checkDBAvailableStatus
     */
    public CheckDBAvailableStatus getCheckDBAvailableStatus() {
        return checkDBAvailableStatus;
    }

    /**
     * Setter method for property <tt>checkDBAvailableStatus</tt>.
     *
     * @param checkDBAvailableStatus value to be assigned to property checkDBAvailableStatus
     */
    public void setCheckDBAvailableStatus(CheckDBAvailableStatus checkDBAvailableStatus) {
        this.checkDBAvailableStatus = checkDBAvailableStatus;
    }
