...
public class SpringBasedRuleMatcherImpl implements Matcher {
	...
	public MatcherResult match(ComparativeMapChoicer comparativeMapChoicer,
							   List<Object> args, LogicTableRule rule) throws TDLCheckedExcption {
		//规则链儿集合，包含了规则中所有规则链
		...
		for (RuleChain ruleChain : ruleChainSet) {
			...
			for (Set<String> oneLevelArgument : requiredArgumentSortByLevel) {
				...
				Map<String/*当前参数要求的列名*/, Comparative> sqlArgs = comparativeMapChoicer.getColumnsMap(args, oneLevelArgument);
		...
		return buildMatcherResult(comparativeMapDatabase, comparativeTable,
				calc);


	}

	public MatcherResult buildMatcherResult(
			Map<String, Comparative> comparativeMapDatabase,
			Map<String, Comparative> comparativeTable, List<TargetDB> targetDB) {
