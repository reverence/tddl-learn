...
public abstract class RowBasedReplicationListener implements SqlExecuteListener{
	...
	//protected final RowBasedReplicationExecutor executor = new RowBasedReplicationExecutor();
	...
	//如果主库主键冲，那么log将留在数据库中，这个以后要进行修正
	...
	public void afterTxCommit(List<SqlExecuteEvent> events) throws SQLException {
		...
		for (SqlExecuteEvent event : events) {
			event.setAfterMainDBSqlExecuteTime(currentTime);
			afterSqlExecute(event);
		}
	}
	...
	protected void insertSyncLog2Db(SqlExecuteEvent event) throws SQLException{
		...
		try {
			this.syncLogDb
					.tryExecute(insertSyncLogTryer, 3, DB_OPERATION_TYPE.WRITE_INTO_DB, event);
	...
	public static class SpecialExceptionPolicy implements ReplicationCallbackHandler {
		public void insertSyncLogFailed(SqlExecuteEvent event, List<SQLException> exceptions) throws SQLException {
			SQLException target = exceptions.size() > 0 ? exceptions.get(0) : null;
			exceptions.add(0, new SaveSyncLogFailedException("insert sync_log db failed", target));
			ExceptionUtils.throwSQLException(exceptions, "insert sync_log sql", null); //TODO null填入event中信息
		}
	}
	/**
