        ...
        if (toTry && lock.tryLock()) {
            try {
                ...
                if (!isFatal || failedDataSources == null) {
                    if (logger.isWarnEnabled()) {
                        logger.warn("异常错误码: " + e.getErrorCode()
                                + " 不是数据库不可用异常或不要求重试，直接抛出.isFatal= " + isFatal
                                + ",failedDataSources.size()=" + failedDataSources.size(), e);
                    }
            ...
            if (logger.isWarnEnabled()) {
                logger.warn("线程" + Thread.currentThread().getName() + "在"
                        + getCurrentDateTime(null) + "时间打算尝试故障数据源" + name
                        + "时失败，其他线程正在访问或者不符合2s的时间间隔！");
            }
            excludeKeys.add(name);
        }
        return t;
    ...
    public <T> T tryOnAvailableDataSource(DB_OPERATION_TYPE operationType,
        ...
        try {
            ...
            if (!isFatal || failedDataSources == null) {
                if (logger.isWarnEnabled()) {
                    logger.warn(
                            "异常错误码: " + e.getErrorCode() + " 不是数据库不可用异常或不要求重试，直接抛出.isFatal= " + isFatal
                                    + ",failedDataSources.size()=" + failedDataSources.size(), e);
                }
                return tryer.onSQLException(exceptions, exceptionSorter, args);
            }
    ...
    public void throwTddlCommnicationException(List<SQLException> exceptions,
        ...
        if (size <= 0) {
            ...
            if (exceptionSorter.isExceptionFatal(lastSQLException)) {
                ...
                exceptions.add(0, new TDDLCommunicationException("tddl communicationException:",
                        lastSQLException));
            }
        }
